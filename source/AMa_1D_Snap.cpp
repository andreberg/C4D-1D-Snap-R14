...
class D1Snap : public DescriptionToolData
{
    public:
        ...
        virtual LONG GetToolPluginId() {
            return ID_AMa_1D_SNAP;
        }
        virtual const String GetResourceSymbol() {
            return String("tool_AMa_1D_Snap");
        }
...
TOOLDRAW D1Snap::Draw(BaseDocument * doc, BaseContainer &data, BaseDraw * bd, BaseDrawHelp * bh, BaseThread * bt, TOOLDRAWFLAGS flags) {
    ...
    // GePrint( "d");
    if (!(flags & TOOLDRAWFLAGS_HIGHLIGHT) || flags & TOOLDRAWFLAGS_INVERSE_Z) {
        return TOOLDRAW_0;
    }
...
#else
                    while (obj->GetPointS()->GetRange(r, &seg_start, &seg_end)) {
#endif
                        ++r;
